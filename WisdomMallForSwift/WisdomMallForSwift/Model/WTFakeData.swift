...
class FakeData {
    ...
    private static var courseList = [Course]()
    ...
    static func createDeals() -> [Deal] {
        if deals.count == 0{
            deals = FakeData.createCourses().map{
                Deal(course: $0, progress: 1)
            }
        }
        return deals
    }
